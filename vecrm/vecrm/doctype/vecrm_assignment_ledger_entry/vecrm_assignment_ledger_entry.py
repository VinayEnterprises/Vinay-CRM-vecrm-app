...
class VECRMAssignmentLedgerEntry(Document):
	"""Standalone cross-doctype ledger of owner-change events.
	istable=0. Written by Lead.before_save on owner change (paired
	with a child-table row in the parent's reassignment_history).
	...
	def on_update(self):
		...
		if self.get_doc_before_save() is not None:
			frappe.throw(
				_("VECRM Assignment Ledger Entry is append-only. Existing entries cannot be modified."),
	...
	def on_trash(self):
		...
		frappe.throw(
			_("VECRM Assignment Ledger Entry is append-only. Entries cannot be deleted."),
			frappe.PermissionError,
		)
