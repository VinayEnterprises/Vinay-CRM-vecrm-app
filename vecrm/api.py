...
def list_employees_directory() -> list[dict]:
    ...
    vecrm_base_city, company, designation}. Same fields the existing
    /travel-vouchers/new submitter dropdown already exposes to admins. No
    auth secrets, no role/status, no email. Strictly the minimum needed to
    humanize the raw phone PK stored in voucher submitter / approver /
    paid_by / rejected_by fields on the list and detail pages — plus the
    company + designation added in S40 (employee convergence) so voucher
    detail pages and the period report can show the submitter's company
    letterhead and designation. Both are non-sensitive HR/display fields.
    ...
    return frappe.get_all(
        "VECRM Employee",
        filters=[["vecrm_account_status", "=", "Active"]],
        fields=["name", "employee_name", "vecrm_base_city", "company", "designation"],
...
def _render_weekly_report_html(data: dict) -> str:
    ...
    sections.append(
        ...
        f'Generated by Anusuya Workspace. Aggregation window: {start} 00:00 → {end} (Asia/Kolkata).'
        f'</div>'
    )
...
def _render_rep_followup_html(rep_name: str, rows: list, today_iso: str) -> str:
    ...
    body.append(
        ...
        f'Anusuya Workspace Daily Reminder</div>'
...
def _render_admin_followup_html(
    ...
    sections.append(
        f'<div style="background:{_HEADER_BG};color:#ffffff;padding:20px 24px;">'
        f'<div style="font-size:11px;letter-spacing:1.5px;text-transform:uppercase;opacity:0.7;">'
        f'Anusuya Workspace Admin Digest</div>'
        f'<div style="font-size:20px;font-weight:600;margin-top:4px;">'
    ...
    sections.append(
        f'<div style="margin-top:32px;padding-top:16px;border-top:1px solid {_BORDER};'
        f'font-size:11px;color:{_MUTED};">'
        f'Generated by Anusuya Workspace. Snapshot date: {today_iso} (Asia/Kolkata). '
