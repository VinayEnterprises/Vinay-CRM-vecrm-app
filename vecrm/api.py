...
def get_my_notifications(limit: int = 50) -> list[dict[str, Any]]:
    ...
    per-row access through the normal permission model. Returns [] when the
    session has no associated email rather than throwing, so the bell degrades
    quietly. Response is shaped to what the portal NotificationBell expects
    (email_content / read), insulating it from the doctype field names.
    """
...
def mark_notification_read(name: str) -> dict[str, Any]:
    ...
    Ownership-guarded: the row's for_email must equal the session's
    vecrm_email, so the shared portal user cannot mark another employee's
    notifications. The write uses ignore_permissions after the guard passes.
    """
    email = frappe.session.data.get("vecrm_email")
    if not email:
        frappe.throw(_("Not authenticated as VECRM Employee"), frappe.PermissionError)
